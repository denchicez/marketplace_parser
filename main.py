...
if(URL.find('wildberries')!=-1):
    ...
    for i in range(1,1000000):
        ...
        index_begin = all_products.find('shortProducts: {')
        index_end = all_products.find('})', index_begin)
        all_products = json.loads(all_products[index_begin+len('shortProducts: '):index_end])
        ...
        if(len(products)==pred):
            break
        pred = len(products)
    # CОХРАНЕНИЕ
    catalog = catalog.replace('https://www.wildberries.ru/catalog/','')
    catalog = catalog.replace('https://www.wildberries.ru','')
